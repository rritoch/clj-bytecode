...
public class T_CONSTANT_Fieldref_info extends T_CPInfo {
	...
	public T_CONSTANT_Fieldref_info() {
		tag = (new T_U1()).add(7);
		class_index = new T_U2();
		name_and_type_index = new T_U2();
	}
	
	private T_CONSTANT_Fieldref_info(T_U2 v,int t, T_CONSTANT_Fieldref_info p) {
		tag = (new T_U1()).add(7);
	...
	public T_CPInfo set(InputStream is) throws IOException {
		T_CONSTANT_Fieldref_info r= new T_CONSTANT_Fieldref_info((new T_U2()).set(is),CLASS_INDEX,this);
		return new T_CONSTANT_Fieldref_info((new T_U2()).set(is),NAME_AND_TYPE_INDEX,r);
	}
}
