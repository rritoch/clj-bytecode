...
public class T_CONSTANT_Double_info extends T_CPInfo {

	/**
	 * 
	 */
	private static final long serialVersionUID = -4463931469935049737L;
	private T_U4 high_bytes;
	private T_U4 low_bytes;

	
	public T_CONSTANT_Double_info() {
		tag = (new T_U1()).add(6);
		high_bytes = new T_U4();
		low_bytes = new T_U4();
	}
	
	private T_CONSTANT_Double_info(T_U4 vh, T_U4 vl) {
		tag = (new T_U1()).add(6);
		high_bytes = vh;
		low_bytes = vl;
	...
	public T_CPInfo set(InputStream is) throws IOException {
		...
		return new T_CONSTANT_Double_info(vh,vl);
	}
